...
void *chooseCommand(void *commandArgs) {
  ...
  int commandLine;
  while((state == NO_BARRIER) && ((commandLine = get_next(args->fd, *args->mutex_get_next)) != EOC)) {
    switch (commandLine){
            ...
            case CMD_SHOW:
              ...
              //fazer um lock que impeça o show e o list de printarem coisas simultaneamente
